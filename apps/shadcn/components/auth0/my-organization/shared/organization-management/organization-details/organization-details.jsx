import { getComponentStyles, createOrganizationDetailSchema } from '@auth0/universal-components-core';
...
export function OrganizationDetails(
  ...
  const currentStyles = React.useMemo(() => getComponentStyles(styling, isDarkMode), [styling, isDarkMode]);

  const organizationDetailSchema = React.useMemo(() => {
    const mergeFieldConfig = (field, defaultError) => {
    ...
    return createOrganizationDetailSchema({
      ...
      displayName: mergeFieldConfig('displayName', t('sections.settings.fields.display_name.error')),
      primaryColor: mergeFieldConfig('primaryColor', t('sections.branding.fields.primary_color.error')),
  ...
  const onValid = React.useCallback(async (values) => {
  ...
  const handlePreviousAction = React.useCallback((event) => {
    form.reset();
