            ...
            {topProducts.map((p) => (
              ...
              >
                <td style={{ padding: "10px 0", borderBottom: "1px solid var(--color-sand-100)", fontSize: 13, verticalAlign: "middle" }}>
                  <div style={{ width: 24, height: 24, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 11, background: p.rank <= 3 ? "var(--color-accent-pale)" : "var(--color-sand-100)", color: p.rank <= 3 ? "var(--color-gray-700)" : "var(--color-gray-400)" }}>
                    {p.rank}
                  </div>
                </td>
                <td style={{ padding: "10px 0", borderBottom: "1px solid var(--color-sand-100)", verticalAlign: "middle" }}>
                  <div style={{ fontFamily: "var(--font-display)", fontWeight: 700, color: "var(--color-gray-800)", fontSize: 14 }}>{p.name}</div>
                  <div style={{ fontSize: 11, color: "var(--color-gray-300)", marginTop: 1 }}>{p.category}</div>
                </td>
                <td style={{ padding: "10px 0", borderBottom: "1px solid var(--color-sand-100)", fontSize: 13, fontVariantNumeric: "tabular-nums", verticalAlign: "middle" }}>{p.views}</td>
